<x-app-layout>
    ...
    <div class="py-12 bg-slate-50">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            @if (session('success'))
                <div class="mb-6 p-4 bg-emerald-50 border border-emerald-200 text-emerald-800 font-semibold text-sm shadow-sm flex items-center gap-2">
                    {{ session('success') }}
                </div>
            @endif

            @if (session('error'))
                <div class="mb-6 p-4 bg-rose-50 border border-rose-200 text-rose-800 font-semibold text-sm shadow-sm flex items-center gap-2">
                    {{ session('error') }}
                </div>
            @endif
            ...
            <div class="mb-6">
                <a href="{{ route('dashboard') }}" class="inline-flex items-center text-sm font-semibold text-emerald-600 hover:text-emerald-700 transition gap-1">
                    ← {{ __('Atpakaļ uz treneru sarakstu') }}
                </a>
            </div>
            ...
            <div class="bg-white border border-gray-200 shadow-sm p-6 mb-8 flex flex-col md:flex-row gap-6 items-center md:items-start">
                ...
                <div class="flex-1 text-center md:text-left">
                    ...
                    <span class="px-3 py-1 bg-emerald-50 text-emerald-700 text-xs font-bold uppercase tracking-wider border border-emerald-200 inline-block mb-4">
                        {{ __('Sertificēts Treneris') }}
                    </span>
                    
                    <div class="mb-4">
                        <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">{{ __('Par mani un kvalifikācija') }}</h4>
                    ...
                    <div>
                        <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">{{ __('Sertifikāti') }}</h4>
                        <p class="text-gray-600 text-sm leading-relaxed font-mono text-xs">
                            {{ $trainer->Sertifikācijas_dati ?? __('Sertifikāti nav norādīti.') }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white border border-gray-200 shadow-sm p-6 mb-8">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                    <div>
                        <h3 class="text-lg font-extrabold text-gray-900 mb-1">{{ __('Pieteikties treniņam') }}</h3>
                        <p class="text-xs text-gray-500">{{ __('Izvēlies datumu, lai apskatītu pieejamos laikus.') }}</p>
                    </div>
                    
                    <span class="text-sm font-extrabold text-slate-700 bg-slate-100 border border-slate-200 px-4 py-2 uppercase tracking-wider">
                        {{ \Carbon\Carbon::now()->locale(app()->getLocale())->isoFormat('MMMM YYYY') }}
                    </span>
                </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(7, minmax(0, 1fr)); gap: 0.75rem;" class="mb-6">
                    ...
                    @for ($i = 0; $i < $daysInMonth; $i++)
                        @php
                            ...
                            $dayName = $currentDate->locale(app()->getLocale())->minDayName;
                ...
                <div id="time-slots-container" class="hidden border-t border-gray-100 pt-6">
                    <h4 class="text-sm font-bold text-gray-900 uppercase tracking-wider mb-4">{{ __('Pieejamie laika logi:') }}</h4>
                            ...
                            <div class="flex items-center">
                                ...
                                <div>
                                    ...
                                    <span class="text-xs ${hasSeats ? 'text-gray-500' : 'text-rose-600 font-bold'}">
                                        ${hasSeats ? '{{ __("Brīvas vietas:") }} ' + s.Max_dalībnieku_skaits : '{{ __("Nav brīvu vietu!") }}'}
                                    </span>
                                </div>
                            </div>
